...
class Runs:
        ...
        while True:
            ...
            for run in runs:
                ...
                if not run.get('name') or not run.get('created_on'):
                    self.logger.log(
                        f'[{self.project["code"]}][Runs] Skipping run {run.get("id", "unknown")} - missing required fields', 'warning')
                    continue
                ...
                if not run_name or run_name.lower() in ['demo', 'demo , will be removed', 'master']:
                    self.logger.log(
                        f'[{self.project["code"]}][Runs] Skipping run "{run_name}" [{run["id"]}] - problematic name', 'warning')
                    continue

                # Skip incomplete TA AN runs
                if run_name.startswith('TA AN:') and len(run_name) < 10:
                    self.logger.log(
                        f'[{self.project["code"]}][Runs] Skipping run "{run_name}" [{run["id"]}] - incomplete TA AN run name', 'warning')
                    continue
    ...
    async def _import_run(self, run: list) -> None:
        try:
            ...
            if not cases_map or len(cases_map) == 0:
                self.logger.log(
                    f'[{self.project["code"]}][Runs] Skipping run {run["name"]} [{run["id"]}] - no cases found', 'warning')
                return

            milestone_id = self.mappings.milestones[self.project['code']][run['milestone_id']
                                                                          ] if run['milestone_id'] in self.mappings.milestones[self.project['code']] else None
            ...
            self.logger.log(
                f'[{self.project["code"]}][Runs] Exception during import of run {run["name"]} [{run["id"]}]: {e}', 'error')
        return
    ...
    async def _import_results_for_run(self, run: list, qase_run_id: str, cases_map: dict) -> None:
        ...
        if should_log_detailed:
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] === DETAILED STATUS LOGGING FOR RUN {run_index} ===')
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Run: {run["name"]} [{run["id"]}] - Qase Run ID: {qase_run_id}')
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Starting import of results for run {run["id"]}')
        ...
        while True:
            ...
            try:
                ...
                self.logger.log(
                    f'[{self.project["code"]}][Runs][STATUS] Raw results from TestRail (offset {offset}): {len(results)} results')
                if not results:
                    self.logger.log(
                        f'[{self.project["code"]}][Runs][STATUS] WARNING: No results returned from TestRail for run {run["id"]} at offset {offset}')
                ...
                if should_log_detailed:
                    ...
                    for result in results:
                        ...
                        status_counts[status_id] = status_counts.get(
                            status_id, 0) + 1
                    self.logger.log(
                        f'[{self.project["code"]}][Runs][STATUS] Status distribution in batch: {status_counts}')

                    # Log first few results for debugging
                    sample_results = results[:3] if len(
                        results) > 3 else results
                    for i, result in enumerate(sample_results):
                        self.logger.log(
                            f'[{self.project["code"]}][Runs][STATUS] Sample result {i+1}: test_id={result.get("test_id")}, status_id={result.get("status_id")}, comment={result.get("comment", "None")[:100] if result.get("comment") else "None"}')

                run_results = run_results + \
                    self._clean_results(results, should_log_detailed)
                offset = offset + limit

                if len(results) < limit:
                    if should_log_detailed:
                        self.logger.log(
                            f'[{self.project["code"]}][Runs][STATUS] Reached end of results (got {len(results)} < limit {limit})')
                    break

            except Exception as e:
                self.logger.log(
                    f'[{self.project["code"]}][Runs] ERROR getting results for run {run["id"]} at offset {offset}: {e}', 'error')
                if should_log_detailed:
                    self.logger.log(
                        f'[{self.project["code"]}][Runs][STATUS] Exception details: {type(e).__name__}: {str(e)}')
                break
        ...
        if should_log_detailed:
            ...
            for result in run_results:
                status_id = result.get('status_id')
                final_status_counts[status_id] = final_status_counts.get(
                    status_id, 0) + 1
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Final status distribution after cleaning: {final_status_counts}')

            if not run_results:
                self.logger.log(
                    f'[{self.project["code"]}][Runs][STATUS] WARNING: No results after cleaning for run {run["id"]}')
        ...
        if should_log_detailed:
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Results after merging comments: {len(run_results)}')
        ...
        if should_log_detailed:
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Results after sorting: {len(run_results)}')

        if not run_results:
            self.logger.log(
                f'[{self.project["code"]}][Runs] WARNING: No results to import for run {run["id"]} - skipping import')
            if should_log_detailed:
                self.logger.log(
        ...
        async with asyncio.TaskGroup() as tg:
            for chunk in self._chunk_list_generator(run_results, 500):
                ...
                tg.create_task(self._import_results(
                    run, qase_run_id, cases_map, chunk, should_log_detailed))
    ...
    def _clean_results(self, results: list, should_log_detailed: bool = False) -> list:
        ...
        if should_log_detailed:
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Cleaning {len(results)} results from TestRail')
        ...
        for result in results:
            ...
            if result['status_id'] != 3:
                ...
                if should_log_detailed:
                    self.logger.log(
                        f'[{self.project["code"]}][Runs][STATUS] Kept result: test_id={test_id}, status_id={original_status_id}')
            else:
                if should_log_detailed:
                    self.logger.log(
                        f'[{self.project["code"]}][Runs][STATUS] Filtered out result: test_id={test_id}, status_id={original_status_id} (status_id=3)')

        if should_log_detailed:
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Cleaned results: {len(clean_results)} kept out of {len(results)} total')
            if len(clean_results) == 0 and len(results) > 0:
                self.logger.log(
                    f'[{self.project["code"]}][Runs][STATUS] WARNING: All results were filtered out! This might indicate an issue with status filtering.')
    ...
    async def _import_results(self, tr_run, qase_run_id, cases_map, results, should_log_detailed: bool = False) -> None:
        if should_log_detailed:
            self.logger.log(
                f'[{self.project["code"]}][Runs][STATUS] Sending {len(results)} results to Qase for run {tr_run["name"]} [{tr_run["id"]}]')
